...
import { api, type UserResponse, type RoleResponse, type AreaResponse } from "../../services/api";
...
function UserModal({ user, roles, areas, onClose, onSaved }: ModalProps) {
  const isEdit = !!user;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit) {
        ...
        if (roleChanged) {
          if (currentRole) {
            await api.users.removeRole(user.id, currentRole.role.id);
          }
          if (roleId) {
            await api.users.assignRole(user.id, roleId, areaId || undefined);
          }
        }
      } else {
        ...
        <div className="p-6">
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  required className="input-dark w-full" placeholder="Juan"
                />
              </div>
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  required className="input-dark w-full" placeholder="Pérez"
                />
              </div>
            </div>
            ...
            {!isEdit && (
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  required className="input-dark w-full" placeholder="usuario@coninmaq.com"
                />
            ...
            <div className="space-y-1.5">
              ...
              <select
                value={roleId} onChange={(e) => { setRoleId(e.target.value); setAreaId(""); }}
            ...
            {areas.length > 0 && (
              <div className="space-y-1.5">
                <label className="text-fg-4 text-xs font-medium uppercase tracking-wider">Área <span className="text-fg-6 normal-case">(opcional)</span></label>
                <select
...
export default function UsersPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [u, r, a] = await Promise.all([api.users.list(), api.roles.list(), api.areas.list()]);
      setUsers(u);
      setRoles(r);
      setAreas(a);
    } finally {
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border">
              {loading && (
                <tr>
                  <td colSpan={6} className="text-center py-12 text-fg-6 text-sm">
                        ...
                        <div className="flex items-center justify-between gap-4">
                          <div className="flex items-center gap-2.5 text-red-300">
                            ...
                            <span className="text-sm">
                              ¿Eliminar a <strong>{u.first_name} {u.last_name}</strong>? Esta acción no se puede deshacer.
                            </span>
                          </div>
