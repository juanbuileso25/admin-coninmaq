...
import { Plus, Pencil, Trash2, AlertTriangle, Loader2, AlertCircle, Building2 } from "lucide-react";
import { api, type AreaResponse } from "../../services/api";
...
function AreaModal({ area, onClose, onSaved }: ModalProps) {
  const isEdit = !!area;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit) {
        await api.areas.update(area.id, { name, description: description || undefined });
      } else {
        await api.areas.create({ name, description: description || undefined });
      }
      onSaved();
    } catch (err: unknown) {
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-4">
            <div className="space-y-1.5">
              ...
              <input
                ...
                required className="input-dark w-full" placeholder="ej. Ventas"
              />
            </div>
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="input-dark w-full" placeholder="Descripción del área"
              />
            </div>
...
export default function AreasPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { setAreas(await api.areas.list()); }
  ...
  const openEdit    = (a: AreaResponse) => { setEditing(a); setModalOpen(true); };
  const closeModal  = () => { setModalOpen(false); setEditing(null); };
  const handleSaved = () => { closeModal(); load(); };

  const confirmDelete = async (id: string) => {
    setDeleteLoading(true);
    try { await api.areas.remove(id); setDeletingId(null); load(); }
    ...
    <div className="space-y-5 max-w-[700px]">
      ...
      <div className="flex items-start justify-between gap-4 animate-fade-up">
        <div>
          ...
          <h1 className="text-fg text-xl font-semibold">Áreas</h1>
        </div>
      ...
      <div className="bg-surface-2 border border-border overflow-hidden animate-fade-up" style={{ animationDelay: "60ms", animationFillMode: "both" }}>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border">
              ...
              {!loading && areas.length === 0 && (
                <tr><td colSpan={3} className="text-center py-10 text-fg-6 text-sm">No hay áreas creadas</td></tr>
              )}
              {!loading && areas.map((a) => (
                <>
                  <tr key={a.id} className="hover:bg-surface-3 transition-colors cursor-pointer group" onClick={() => openEdit(a)}>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2.5">
                        <Building2 size={14} className="text-accent opacity-70 flex-shrink-0" />
                        <span className="text-fg-2 font-medium text-sm">{a.name}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-fg-5 text-xs">{a.description ?? "—"}</td>
                  ...
                  {deletingId === a.id && (
                    <tr key={`del-${a.id}`} className="bg-red-950/20 border-b border-red-900/30">
                      <td colSpan={3} className="px-4 py-3">
                        <div className="flex items-center justify-between gap-4">
                          <div className="flex items-center gap-2.5 text-red-300">
                            <AlertTriangle size={15} />
                            <span className="text-sm">¿Eliminar el área <strong>{a.name}</strong>?</span>
                          </div>
                          <div className="flex items-center gap-2 flex-shrink-0">
                            ...
                            <button onClick={() => confirmDelete(a.id)} disabled={deleteLoading}
        ...
        <div className="px-4 py-3 border-t border-border bg-surface-3">
          <p className="text-fg-6 text-xs"><span className="text-fg-4">{areas.length}</span> áreas en total</p>
        </div>
      </div>

      {modalOpen && <AreaModal area={editing} onClose={closeModal} onSaved={handleSaved} />}
    </div>
  );
}
