...
interface Props {
  maintenance: RentalMaintenanceResponse;
  onClose: () => void;
  onChange: () => void;
}

export default function MaintenanceAttachmentsPopover({ maintenance, onClose, onChange }: Props) {
  ...
  async function handleUpload(file: File) {
    setUploading(true);
    try {
      await api.rental.uploadMaintenanceAttachment(maintenance.id, file);
      onChange();
    } finally {
      setUploading(false);
    }
  ...
  async function handleDelete(id: string) {
    ...
    await api.rental.deleteMaintenanceAttachment(id);
    onChange();
  }
